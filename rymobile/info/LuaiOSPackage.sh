...
cocos luacompile -s $Cur_Dir/../client/client -d $Cur_Dir/../ciphercode/client -e -k ZK_RY_IHIHVIP_!2020 -b ZK_IHIH_QQ2020 --disable-compile
if [ $? -eq 1 ]
then
CipherSrcError "lua文件加密出错"
fi

cocos luacompile -s $Cur_Dir/../client/game -d $Cur_Dir/../ciphercode/game -e -k ZK_RY_IHIHVIP_!2020 -b ZK_IHIH_QQ2020 --disable-compile
if [ $? -eq 1 ]
then
CipherSrcError "lua文件加密出错"
fi

cocos luacompile -s $Cur_Dir/../client/base -d $Cur_Dir/../ciphercode/base -e -k ZK_RY_IHIHVIP_!2020 -b ZK_IHIH_QQ2020 --disable-compile
if [ $? -eq 1 ]
then
CipherSrcError "lua文件加密出错"
fi
