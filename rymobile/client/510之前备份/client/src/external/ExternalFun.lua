...
--捕鱼
function ExternalFun.strFormatScore( llScore, widget, isNotFloat )

    local multiple  = 100
    if 'number' ~= type(llScore) then
        return llScore
    end
    local score     = llScore / multiple
    local strFormat = "%0.2f"
    local strTenThousand    = ""
    local strHundredMillion = ""
    ...
    if description then
        if description == "TextAtlas"  or 
            string.sub(description, 2, 11) == "LabelAtlas" then
            strTenThousand      = ":"
            strHundredMillion   = ";"
        else
            strTenThousand      = "万"
    ...
    if math.abs(score) > 9999.99 then
--        score       = score / 10000
--        strFormat   = strFormat..strTenThousand
    elseif math.abs(score) > 99999999.99 then
--        score       = score / 100000000
--        strFormat   = strFormat..strHundredMillion
    end
    local strScore  = string.format(strFormat, tonumber(score))
