...
function ClientScene:onCreate()
	...
	if self._areaTrumpet == nil then
		...
		self._areaTrumpet:addTrugTxt("510游戏，提倡健康休闲的游戏理念，严禁赌博、严禁私下交易")
	end
	...
	self.btnBank:addClickEventListener(function()
		...
		if GlobalUserItem.cbInsureEnabled == 0 then
			showPopupLayer(BankEnableLayer:create(function()
				BankLayer:create(self:getApp()._funList.GiveModule):addTo(self)
			end))
		else
			BankLayer:create(self:getApp()._funList.GiveModule):addTo(self)
		end
	...
	for i=1,6 do
	 ...
	 --棋牌按钮特效
	jsonName = 'plaza/spine/lhd.json'
	atlasName = 'plaza/spine/lhd.atlas'
...
function ClientScene:NoticeCallBack( event )
	...
	if self._areaTrumpet ~=nil then
		self._areaTrumpet:addTrugTxt(msg)
	end
