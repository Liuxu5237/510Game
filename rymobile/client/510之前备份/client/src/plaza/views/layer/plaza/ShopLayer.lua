...
function ShopLayer:ctor(listType)
	...
	local pos = cc.p(btngold:getPosition())
	btngold:setPosition(btnbean:getPosition())
...
function ShopLayer:GetGroptype(event)
	...
	--local typeID = a.PayType
	-- if typeID == PropertyType.Bean then
	-- 	--保存
	-- 	GlobalUserItem.tabShopCache.shopBeanList = list
	-- 	--更新列表
	-- 	self:updateList(ListType.Bean)		
	-- elseif typeID == PropertyType.Gold then			
	-- 	--保存
...
function ShopLayer:getListItem(listType, index)
	...
	elseif listType == ListType.Gold then           --游戏币		
		...
		if index == self:getListCount(listType) then
			...
			itemCount = GlobalUserItem.dUserBeans *(itemInfo.BuyResultsGold / itemInfo.Cash) --游戏豆数量乘以第一个等级兑换的游戏币数量
