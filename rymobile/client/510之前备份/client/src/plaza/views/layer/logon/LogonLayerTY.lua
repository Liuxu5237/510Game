...
local LogonLayerTY = class("LogonLayerTY", cc.Layer)
...
function LogonLayerTY:ctor(delegate)

    self._delegate = delegate

    local csbNode = ExternalFun.loadCSB("Logon/LogonLayerTY.csb"):addTo(self)
...
--    logo:setVisible(false)
    ...
    self._editAccount:setVisible(false)
    self._editPassword:setVisible(false)
...
--   self._editPassword:registerScriptEditBoxHandler(editHanlder)
    ...
    self._checkRemember:setVisible(false)
...
function LogonLayerTY:onEditEvent(event,editbox)
...
--创建输入框
function LogonLayerTY:onCreateEditBox(spEditBg, isPassword, isNumeric, maxLength)
...
function LogonLayerTY:editboxEventHandler(eventType,sender)
...
--忘记密码
function LogonLayerTY:onClickForget()
...
--注册
function LogonLayerTY:onClickRegister()

    showPopupLayer(RegisterLayer:create(self._delegate), false)
end

--登录
function LogonLayerTY:onClickLogon()
...
--验证登陆提示      
function LogonLayerTY:onChenckUsertips(unidUserID)
