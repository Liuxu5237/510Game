...
function ModifyFrame:sendModifyUserInfo()
	...
	ModifyUserInfo:pushstring(GlobalUserItem.szPassword,33)
...
function ModifyFrame:sendModifyLogonPass()
	local ModifyLogonPass = CCmd_Data:create(196)	
	...
	ModifyLogonPass:pushstring(GlobalUserItem.szAccount,32)
	ModifyLogonPass:pushstring(self._szLogonNew,yl.LEN_MD5) 
	ModifyLogonPass:pushstring(self._szLogonOld,yl.LEN_MD5)
...
function ModifyFrame:sendModifyBankPass()
	local ModifyInsurePass = CCmd_Data:create(196)
	...
	ModifyInsurePass:pushstring(GlobalUserItem.szAccount,32)
	ModifyInsurePass:pushstring(GlobalUserItem.PasswordEncryption(self._szBankNew),33)
	ModifyInsurePass:pushstring(GlobalUserItem.PasswordEncryption(self._szBankOld),33)
...
function ModifyFrame:sendAccountBinding()
	...
	AccountBinding:pushstring(GlobalUserItem.szPassword,33)
...
function ModifyFrame:sendModifySystemFace( )
	...
	sysmodify:pushstring(GlobalUserItem.szPassword,yl.LEN_MD5)
	...
	if message[2] == 112 then
		...
		if message[3]["code"] == 0 then
			...
			if self._oprateCode == 0 then
				...
				if nil ~= self._callBack then
					self._callBack(1,"修改成功")
				end	
			elseif self._oprateCode == MODIFY_SYSTEM_FACE then
				GlobalUserItem.wFaceID = self._wFaceId
				if nil ~= self._callBack then
					self._callBack(yl.SUB_GP_USER_FACE_INFO,"头像修改成功")
				end	
			end
...
function ModifyFrame:onModifyUserInfo(cbGender,szNickname,szSign)
	...
	self._wFaceId = GlobalUserItem.wFaceID
	...
	self._webFrame = WebFrame:create(self, function(result, message)
		if self.webOperateCallBack then
			self:webOperateCallBack(result,message)
		end
	end)
	
	local action = {}
	action["Gender"] = self._cbGender
	action["wFaceID"] = self._wFaceId
	action["UnderWrite"] = self._szSign
	
    local pargams = {1,12,action}
...
function ModifyFrame:onModifyLogonPass(szOld,szNew)
	...
	self._szLogonOld = GlobalUserItem.PasswordEncryption(szOld)
	self._szLogonNew = GlobalUserItem.PasswordEncryption(szNew)
	...
	local action = {}
	action["P"] = self._szLogonOld
	action["PN"] = self._szLogonNew
    local pargams = {1,13,action}
    local action2 = cjson.encode(pargams)
    self._webFrame:wsSendString(action2) 
...
function ModifyFrame:onModifyBankPass(szOld,szNew)
	self._oprateCode = 2
	self._szBankOld = GlobalUserItem.PasswordEncryption(szOld)
	self._szBankNew = GlobalUserItem.PasswordEncryption(szNew)
...
function ModifyFrame:onModifySystemHead( wFaceId )
	...
	self._szMachine = GlobalUserItem.szMachine
	self._cbGender = GlobalUserItem.cbGender
	self._szSign   = GlobalUserItem.szSign 
	...
	local action = {}
	action["Gender"] = GlobalUserItem.cbGender
	action["wFaceID"] = self._wFaceId
	action["UnderWrite"] = self._szSign
