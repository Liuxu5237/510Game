...
function GameServerItem:onInit(pData,i)
	if pData == nil then
	...
	self.wKindID = pData["wKindID"][i]				--名称索引
	self.wServerID = pData["wServerID"][i]			--房间索引
	self.wTableCount = pData["T"][i]				--房间类型
	self.szServerName = pData["szServerName"][i]	--房间名称
	self.lCellScore =  pData["lCellScore"][i]
	
	self.lEnterScore =  pData["lBaseScore"][i]
