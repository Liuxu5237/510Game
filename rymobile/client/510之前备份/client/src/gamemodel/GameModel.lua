...
function GameModel:setSearchPath()
    ...
    if nil ~= entergame then
        ...
        if LOCAL_DEVELOP == 1 then
            self._searchPath = "game/" .. modulestr .. "/res/"
        else
...
--退出询问
function GameModel:onQueryExitGame(str)
    if PriRoom and true == GlobalUserItem.bPrivateRoom then
        ...
        self._queryDialog = QueryDialog:create(str, function(ok)
            if ok == true then
                --退出防作弊
...
-- 退出桌子
function GameModel:onExitTable(_time)
        ...
        self:runAction(cc.Sequence:create(
            cc.CallFunc:create(
                function () 
                    ...
                    if WebFrame:getSocketState()==1 then
            ...
            cc.CallFunc:create(
                function ()
                    ...
                    if WebFrame:getSocketState()==1 then
                        ...
                        local eventListener = cc.EventCustom:new("ry_GameLeaveForWebClose")
