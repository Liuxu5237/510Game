...
function GameNotice:ctor(msg,pos)
	...
	self._layout:setPosition(cc.p(237,15))
	...
	self._trutxt:setPosition(cc.p(20,10))
...
function GameNotice:addTrugTxt(msg)
	table.insert( self.txtList ,msg )
end

--取消消失
function GameNotice:showTrugTxt(msg)
	...
	--初始化位置
	self._trutxt:setPosition(containerWidth, 15)
	...
	self._trutxt:runAction(
		--cc.RepeatForever:create(
			cc.Sequence:create(
				cc.CallFunc:create(function() self._trutxt:setPosition(containerWidth, 10) end),
				cc.MoveBy:create(7 + contentSize.width / 172, cc.p(-contentSize.width - containerWidth, 0))
