...
--聊天文本大小
local SIZE_CHAT_LABEL = 26
--聊天内容宽度
local LEN_CHAT_WIDTH = 430
...
function GameChatLayer:ctor(netframe, csbfile)
	...
	csbfile = csbfile or "public/GameChatLayer_DDZ.csb"
	--加载csb资源
	...
	--表情
    cbt = chat_bg:getChildByName("brow_check")
	cbt:setTag(CBT_BROW)
    cbt:addEventListener(checkEvent)
    cbt:setSelected(false)
	self.m_browRecord = cbt   
...
function GameChatLayer:showBrowPanel(bShow)
	if bShow then

		if nil == self.m_browTableView then
			local function broCallback(ref, tType)
				if tType == ccui.TouchEventType.ended then
					 self:sendBrowChat(ref:getTag())
				end
			end

			self.m_browTableView =ccui.ScrollView:create()
			self.m_browTableView:setDirection(ccui.ScrollViewDir.vertical)
			self.m_browTableView:setAnchorPoint(cc.p(0,0))
			self.m_browTableView:setBounceEnabled(true)
			self.m_browTableView:setScrollBarEnabled(false)
			self.m_browTableView:setInnerContainerSize(cc.size(410,800))
			self.m_browTableView:setContentSize(cc.size(410, 290))
			self.m_browTableView:setPosition(cc.p(22,32))
			self.m_spChatBg:addChild(self.m_browTableView)
			for i=0,79 do
				local str = string.format( "e(%d).png",i )
				local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(str)
				--local _bro1 = cc.Sprite:createWithSpriteFrame(frame)
				--local _bro = ccui.ImageView:create()
				--_bro:loadTexture(_bro1,1)
				local _bro = ccui.Button:create(str,str,str,ccui.TextureResType.plistType)
				local  a,b =  math.modf(i/BRPW_CELL)
				_bro:setTag(i)
				_bro:addTouchEventListener(broCallback)
				_bro:setPosition(cc.p(30+(i%BRPW_CELL)*58,780-65*(a)))
				--self.m_spChatArea:addChild(_bro)
				self.m_browTableView:addChild(_bro)
...
function GameChatLayer:showTextChat(bShow)
	if bShow then
		if nil == self.m_textTableView then
			self.m_textTableView = self:getDataTableView(cc.size(LEN_TABLEVIEW_WIDTH, 290))
...
function GameChatLayer:showRecord(bShow)
	if bShow then
		if nil == self.m_recordTableView then
			self.m_recordTableView = self:getDataTableView(cc.size(LEN_TABLEVIEW_WIDTH, 450))
...
function GameChatLayer:getDataTableView(size)
	...
	m_tableView:setPosition(cc.p(29,36))
...
function GameChatLayer:cellSizeForTable( view, idx )
	if CBT_TEXT == self.m_nSelect then
		...
		if nil ~= record then
			return 380, 73--self:getTextChatSize(record.strChat)
		end
		return 0,0
...
function ChatItem:ctor( bRecord )
    if not bRecord then
        ...
        if nil ~= frame then
			...
			self.m_spLine = cc.Sprite:create("public/Chat/lineBG.png")
...
function ChatItem:refreshTextItem(str,sendusernick)
	if nil == self.m_labelChat then
		...
		self.m_labelChat:setPosition(35, self.m_spLine:getContentSize().height * 0.5)
		--self.m_labelChat:setDimensions(cc.size(380,27))
		self:addChild(self.m_labelChat)
	else
		self.m_labelChat:setString(str)
	end
	local labSize = self.m_labelChat:getContentSize()
	self:setContentSize(cc.size(LEN_TABLEVIEW_WIDTH, labSize.height))
end
