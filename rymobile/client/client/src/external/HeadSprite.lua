...
function HeadSprite:updateHead( useritem )
	...
	--useritem.szThirdPartyUrl = "http://thirdwx.qlogo.cn/mmopen/vi_32/PiajxSqBRaEJuZtpjT9evaIhfQ8KDXdiahicshndcSzZELkS0XnhtiaTegEnSbaxCiamJBb4Nib6opUxV2XFjCrYPSOA/132"
	--系统头像
	local faceid = useritem.wFaceID or 0
	if true == bAntiCheat then
		faceid = 0
	elseif nil ~= useritem.szThirdPartyUrl and string.len(useritem.szThirdPartyUrl) > 0 then
...
function HeadSprite:updateHeadByFrame(frame)
	...
	self:setContentSize(self.m_spRender:getContentSize())
	self.m_fScale = self.m_headSize / self.m_spRender:getContentSize().width--SYS_HEADSIZE
	self:setScale(self.m_fScale)
end
...
function HeadSprite:haveCacheOrLocalFile(framename, filepath, bmpfile)
	...
	if nil ~= frame then
		...
		if cc.FileUtils:getInstance():isFileExist(path) then
			...
			if nil ~= customframe then
				self.m_useritem.dwCustomID = self.m_useritem.dwCustomID or 0
				--缓存帧
...
function HeadSprite:downloadFace(url, path, filename, onDownLoadSuccess)
	...
	appdf.onHttpImage(url, path, filename, cc.exports.g_FaceDownloadListener)
