...
PLATFORM[2001] = appdf.EXTERNAL_SRC .. "platform.Bridge_ios"
...
function MultiPlatform:getMachineId()
	...
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getMachineId then
		...
		if cc.PLATFORM_OS_WINDOWS == targetPlatform and getWinMachineId then
			local MachineId = getWinMachineId()
			return md5(MachineId)
		end
...
function MultiPlatform:getMachineId2()
	local plat = self:getSupportPlatform()
	
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getMachineId2 then
		return g_var(PLATFORM[plat]).getMachineId2( )
	else
		if cc.PLATFORM_OS_WINDOWS == targetPlatform and getWinMachineId then
			local MachineId = getWinMachineId()
			return md5(MachineId)
...
--获取设备id
function MultiPlatform:getMachineIdByZ()
	local plat = self:getSupportPlatform()
	
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getMachineIdByZ then
		return g_var(PLATFORM[plat]).getMachineIdByZ( )
	else
		if cc.PLATFORM_OS_WINDOWS == targetPlatform and getWinMachineId then
			local MachineId = getWinMachineId()
			return md5(MachineId)
...
--初始化爱悦玩游戏
function MultiPlatform:initAWYWithLogin( callback )
	
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).initAWYWithLogin then
		return g_var(PLATFORM[plat]).initAWYWithLogin( callback)
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return 0, msg
...
--爱玩悦zf游戏
function MultiPlatform:AWYZF_z( thirdparty, payparamTab, callback )
	if nil == callback or type(callback) ~= "function" then
		return false, "need callback function"
	end
	print("爱悦玩支付1")
	...
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).AWYZF_z then
		return g_var(PLATFORM[plat]).AWYZF_z( thirdparty, payparamTab, callback)
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return 0, msg
...
--登出
function MultiPlatform:loginOutAWY(  )
	
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).loginOutAWY then
		return g_var(PLATFORM[plat]).loginOutAWY( )
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return 0, msg
...
function MultiPlatform:openWX(  )
	
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).openWX then
		return g_var(PLATFORM[plat]).openWX( )
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return 0, msg
