...
function Bridge_ios.getMachineId()
    ...
    if not ok then
        ...
        print("The md5 ret is:" .. md5(ret))
        return md5(ret)
    end
end

function Bridge_ios.getMachineId2()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"getUUID")
    if not ok then
        print("luaj error:" .. ret)
        return "A501164B366ECFC9E249163873094D50"
...
function Bridge_ios.thirdPartyPay(thirdparty, payparamTab, callback)
    ...
    payparamTab.info = payparamTab.info

    local ok,ret  = luaoc.callStaticMethod(BRIDGE_CLASS,"thirdPartyMai",payparamTab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return false, msg   
...
function Bridge_ios.initAWYWithLogin( callback )
    local paramtab = {scriptHandler = callback}
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"initAWYWithLogin", paramtab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
...
function Bridge_ios.AWYZF_z( thirdparty, payparamTab, callback )
    payparamTab._nidx = thirdparty
    payparamTab.scriptHandler = callback
    ...
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"AWYZF_z", payparamTab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
...
function Bridge_ios.loginOutAWY(  )
    
    local paramtab = {msg = "111"}
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"loginOutAWY", paramtab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
...
function Bridge_ios.openWX(  )
    
    local paramtab = {msg = "111"}
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"openWX", paramtab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
