...
--发牌位置
--local cardpoint = {cc.p(585, 682), cc.p(196, 245), cc.p(196+246, 245), cc.p(196+246*2, 245), cc.p(196+246*3,245)}
local cardpoint = {cc.p(585, 682), cc.p(124, 245), cc.p(124+288, 245), cc.p(124+288*2, 245), cc.p(124+288*3,245)}
--自己头像位置
local selfheadpoint = cc.p(63, 57)
--庄家头像位置
local bankerheadpoint = cc.p(360, 663) 
--玩家列表按钮位置
...
function GameViewLayer:loadResource()
    ...
    self._laba:addTo(self.m_scbNode,99)
    ...
    local layout_bg = self.m_scbNode:getChildByName("im_game_bg")
    ...
    --超级抢庄
    self.m_btSupperRob = self.m_bankerbg :getChildByName("bt_supperrob")
    ...
    for i=1,6 do
        ...
        btn:loadTextureNormal(string.format("client/res/Jetton/Button_chip_%d.png", self.m_pJettonNumber[i].k), 0)
		--btn:loadTexturePressed(string.format("client/res/Jetton/Button_chip_%d_se.png", self.m_pJettonNumber[i].k), 0)
		btn:loadTextureDisabled(string.format("client/res/Jetton/Button_chip_%d_dis.png", self.m_pJettonNumber[i].k), 0)
    ...
    self.m_JettonLight:loadTexture("client/res/Jetton/bt_score_light.png")
    ...
    for i=1,4 do
        ...
        self.m_tAllJettonScore[i] = cc.LabelBMFont:create(" ", "battleOx.fnt") 
        local tms = btn:getChildByName("txt_score")
        self.m_tAllJettonScore[i]:setPosition(tms:getPosition())
        self.m_tAllJettonScore[i]:setAnchorPoint(tms:getAnchorPoint())
        self.m_tAllJettonScore[i]:setVisible(false)
        btn:addChild(self.m_tAllJettonScore[i])
    ...
    for i=1,4 do
        ...
        self.m_selfJettonScore[i] = cc.LabelBMFont:create(" ", "battleOx.fnt")  
        local tms = btn:getChildByName("txt_score")
        self.m_selfJettonScore[i]:setPosition(cc.p(tms:getPositionX(),tms:getPositionY()-4))
        self.m_selfJettonScore[i]:setAnchorPoint(tms:getAnchorPoint())
        btn:addChild(self.m_selfJettonScore[i])
    ...
    for i=1,5 do
        --赢的效果
        if i >= 2 then
            self.m_winEffect[i] = ccui.ImageView:create("game_res1/im_effect_1.png", 0)
            self.m_winEffect[i]:setPosition(self.m_JettonArea[i-1]:getPosition())
            self.m_winEffect[i]:setScaleX(1.1)
        ...
        --输的效果
        if i >= 2 then
            self.m_failedEffect[i] = ccui.ImageView:create("game_res1/im_effect_0.png", 0)
            self.m_failedEffect[i]:setPosition(self.m_JettonArea[i-1]:getPosition())
            self.m_failedEffect[i]:setVisible(false)
            self.m_cardLayer:addChild(self.m_failedEffect[i])
        end

        self.m_niuPointLayout[i] = ccui.ImageView:create("im_blank.png")
...
function GameViewLayer:initBankerInfo()
    ...
    local pbankername = ClipText:createClipText(cc.size(130, 26), "无人坐庄");
    ...
    self.m_bankerScore = ccui.Text:create("", "fonts/round_body.ttf", 22) 
    self.m_bankerScore:setColor(cc.c3b(255, 255, 0))
    local tms = self.m_bankerbg:getChildByName("txt_score")
    self.m_bankerScore:setPosition(cc.p(tms:getPositionX(),tms:getPositionY()))
    self.m_bankerScore:setAnchorPoint(tms:getAnchorPoint())
    self.m_bankerbg:addChild(self.m_bankerScore)
...
function GameViewLayer:resetBankerInfo()
    ...
    local temp = self.m_bankerScore
    if self.m_wBankerUser == yl.INVALID_CHAIR then
        local phead = HeadSprite:createNormal(nil, 82)
        phead:setPosition(127,70)
        ...
        if nil ~= userItem then
            ...
            local temp = PopupInfoHead:createNormal(userItem, 82)
            temp:setPosition(127, 70)
            temp:setTag(TAG_ENUM.HEAD_BANKER)
            self.m_bankerbg:addChild(temp)
            temp:enableInfoPop(false, cc.p(bankerheadpoint.x, 280), cc.p(0, 1))
        end
    end
end
...
function GameViewLayer:initSelfInfo()
    local temp = PopupInfoHead:createClipHead(GlobalUserItem, 88)
    temp:setPosition(62,59)
    self.m_selfbg:addChild(temp)
    temp:enableInfoPop(false)
    ...
    self.m_selfScore = ccui.Text:create("", "fonts/round_body.ttf", 22) 
    self.m_selfScore:setColor(cc.c3b(255, 255, 0))
    local tms = self.m_selfbg:getChildByName("txt_score")
    self.m_selfScore:setPosition(cc.p(tms:getPositionX(),tms:getPositionY()))
    self.m_selfScore:setAnchorPoint(tms:getAnchorPoint())
    self.m_selfbg:addChild(self.m_selfScore)
    self.m_selfScore:setString(ExternalFun.numberThousands(self.m_showScore))
    tms:removeFromParent()
...
function GameViewLayer:onButtonClickedEvent(tag, ref)
    ...
    elseif TAG_ENUM.BT_CLOSEHELP == tag then
        if nil ~= self.m_helpLayer then
            self.m_helpLayer:setVisible(false)
        end
    elseif TAG_ENUM.BT_CLOSEHELP == tag then --关闭帮助
        self:showMenu()
    ...
    elseif tag == TAG_ENUM.BT_APPLY then
        ...
        self.m_applyListLayer:refreshScore(self.m_llCondition)
...
function GameViewLayer:onJettonAreaClicked(tag, ref)
    ...
    local selfscore  = jettonscore *MaxTimes
...
function GameViewLayer:onGameEnd(cmd_table)
    ...
    --print("牌型 ： " self.m_cbTableCardArray)
    local bankername = "系统"
    if  self.m_wBankerUser ~= yl.INVALID_CHAIR then
        local useritem = self:getDataMgr():getChairUserList()[self.m_wBankerUser + 1]
        if nil ~= useritem then
...
function GameViewLayer:onPlaceJettonFail(cmd_table)
    if self:isMeChair(cmd_table.wPlaceUser) == true then
        --change by zzy
        showToast(self, "达到自己下注上限，无法下注", 1)
...
function GameViewLayer:showCard(banim)
    ...
    for j=1,5 do
        --print("开拍 " .. j .."aaaa")
        --dump(self.m_cbTableCardArray[j])
    end
    ...
    for i=1,5 do
        ...
        local lTempCard = clone(self.m_cbTableCardArray[i])
        local niuvalue = GameLogic:GetCardType(lTempCard, 5)
        ...
        local pbglayout = self.m_niuPointLayout[j]
        --pbglayout:loadTexture("im_point_failed_bg.png", UI_TEX_TYPE_PLIST)

        --local ptemp = self:getNiuTxt(string.format("game_res/ox_%d.png",tonumber(niuvalue)), niuvalue)
        local ptemp = cc.Sprite:create()
        --恢复 一会

        --我只是个酱油 获取下数据而已别激动
         local lDataTempCard = clone(self.m_cbTableCardArray[j])
         local  Dataniuvalue = GameLogic:GetCardType(lDataTempCard, 5)


        ptemp:setTexture(string.format("game_res/ox_%d.png",tonumber(Dataniuvalue)-1))
        ptemp:setPosition(pbglayout:getContentSize().width/2, pbglayout:getContentSize().height/2-20)
...
--            local pwinscorebg = ccui.ImageView:create("game_res1/im_jetton_score_bg.png", 0)
--            pwinscorebg:setAnchorPoint(0.5, 1)
--            pwinscorebg:setPosition(pbglayout:getContentSize().width/2, -70)
...
--            pwinscore:setPosition(pwinscorebg:getContentSize().width/2, 20)
         ...
         if value == 6 then
            ...
            if pbglayout ~= nil then
             ...
             for v=1,5 do
              ...
              if v >= 2 then
                --  闲家赢
                if self.m_bUserOxCard[v] == 1 then
                    self.m_winEffect[v]:setVisible(true)
                    self.m_winEffect[v]:runAction(cc.RepeatForever:create(cc.Blink:create(1.0,1)))
                else
                    self.m_failedEffect[v]:setVisible(false)
                end
            end
            ...
            if v>1 then
             ...
             local pbglayout = self.m_niuPointLayout[v]
            ...
            pwinscorebg:addChild(pwinscore)
              ...
              local a, b = GameLogic:CompareCard(self.m_cbTableCardArray[1], self.m_niuPoint[1], self.m_cbTableCardArray[v], self.m_niuPoint[v])


                 if self.m_lUserJettonScore[v] > 0 then
                local str = ExternalFun.formatScoreText(self.m_lUserJettonScore[v]*b)
                if a  == 1 then
                    str = "+"..str
                else
                    pwinscore:setColor(cc.c3b(30, 140, 210))
        ...
        if banim then
            self.m_cardLayer:runAction(cc.Sequence:create(cc.DelayTime:create((i-1)*1.2), cc.CallFunc:create(
                function ()
                    ...
                    local soundValue = self.m_niuPoint[j] - 1
...
function GameViewLayer:showUserJetton(cmd_table)
    ...
    --     if goldnum ~= self.m_pJettonNumber[i].k then
    --         a = a+1
    --     end
    -- end
    -- if a>= #self.m_pJettonNumber then
    ...
    for i=1,goldnum do
       ...
       for i=1,3 do
            ...
            local b  = math.random(1,#self.m_pJettonNumber)
            goldnum = self.m_pJettonNumber[b].k
            --local pgold = cc.Sprite:create()
            local pgold = cc.Sprite:createWithSpriteFrameName(string.format("ChouMa_%d.png",goldnum))
            --pgold:setTexture(string.format("client/res/Jetton/ChouMa_%d.png",goldnum))
    ...
    if self:isMeChair(cmd_table.wChairID) then
        ...
        self.m_showScore = self.m_showScore - tonumber(GameViewLayer.m_BTJettonScore[self.m_nJettonSelect]);
        print("下注 金额 ：  " .. tonumber(GameViewLayer.m_BTJettonScore[self.m_nJettonSelect]))
        if self.m_showScore < 0 then
            self.m_showScore = 0
        end
        self:updateAreaScore(false)
        self:updateJettonList(self.m_showScore)
		self:resetSelfInfo()
...
function GameViewLayer:showGoldToArea()
    ...
    if failureScore == 0 then
        ...
        self:runAction(cc.Sequence:create(cc.DelayTime:create(2.0), cc.CallFunc:create(
            function ()
                self:showGameEnd()
            end
        )))
...
function GameViewLayer:showGoldToUser()
    ...
    self:runAction(cc.Sequence:create(cc.DelayTime:create(3.0), cc.CallFunc:create(
            function ()
                self:showGameEnd()
            end
        )))
...
function GameViewLayer:GoldMoveToUserDeal(cbArea, goldNum, endpos)
    ...
    for i=1,goldNum do
        ...
        pgold:runAction(cc.Sequence:create(cc.DelayTime:create(i*0.005), cc.CallFunc:create(
                function ()
...
function GameViewLayer:showGameEnd(bRecord)
    ...
    if bRecord == nil then
      ...
      --
        ...
        --  嗯哼我就不循环我就喜欢这样写
            local a, b = GameLogic:CompareCard(self.m_cbTableCardArray[1], self.m_niuPoint[1], self.m_cbTableCardArray[2], self.m_niuPoint[2])
            local a1, b1 = GameLogic:CompareCard(self.m_cbTableCardArray[1], self.m_niuPoint[1], self.m_cbTableCardArray[3], self.m_niuPoint[3])
            local a2, b2 = GameLogic:CompareCard(self.m_cbTableCardArray[1], self.m_niuPoint[1], self.m_cbTableCardArray[4], self.m_niuPoint[4])
            local a3, b3 = GameLogic:CompareCard(self.m_cbTableCardArray[1], self.m_niuPoint[1], self.m_cbTableCardArray[5], self.m_niuPoint[5])
            record.bWinTianMen = (a== -1) and 0 or 1
            record.bWinDiMen =  (a1 == -1) and 0 or 1
            record.bWinXuanMen = (a2 == -1) and 0 or 1
            record.bWinHuangMen = (a3== -1) and 0 or 1

            if record.bWinTianMen == 0 then
               record.bWinTianMen = false
               else
               record.bWinTianMen = true
            end

            
            if record.bWinDiMen == 0 then
               record.bWinDiMen = false
               else
               record.bWinDiMen = true
            end

            
            if record.bWinXuanMen == 0 then
               record.bWinXuanMen = false
               else
               record.bWinXuanMen = true
            end

            
            if record.bWinHuangMen == 0 then
               record.bWinHuangMen = false
               else
               record.bWinHuangMen = true
            end
...
function GameViewLayer:convScoreToWanUnit( label_score,score )
    local scc = ""
    if tonumber(score)>10000.0 then
       local ex = score / 10000.0
        scc = string.format("%.2f;",ex)
        scc = tostring(scc)
        scc = string.gsub(scc,"[.]",":")
    else
        scc = tostring(score)
...
function GameViewLayer:updateAreaScore(bshow)
    ...
    for i=1,4 do
        if self.m_lUserJettonScore[i+1] > 0 then
            ...
            self:convScoreToWanUnit(self.m_selfJettonScore[i],self.m_lUserJettonScore[i+1])
        ...
        if self.m_lAllJettonScore[i+1] > 0 then
            ...
            self:convScoreToWanUnit(self.m_tAllJettonScore[i],self.m_lAllJettonScore[i+1])
        end
    end
end
...
function GameViewLayer:getWinGoldNumWithAreaID(cbArea)
    ...
    --print(" all Jetton Score : " .. lAllJettonScore .. " nmultiple :  " .. nmultiple)
    --dddddd    报错 为空 getWinGoldNum（参数为空）
    --暂时处理 未找到为什么参数是空
    if lAllJettonScore == nil or nmultiple == nil  then
        lAllJettonScore=0
        nmultiple=0
    end
...
function GameViewLayer:createHelpLayer()
    ...
    self:addToRootLayer(self.m_helpLayer, ZORDER_LAYER.ZORDER_Other_Layer)
    ...
    local function btnEvent( sender, eventType )
     ...
     --加载csb资源
     local csbNode = ExternalFun.loadCSB("HelpLayer.csb", self.m_helpLayer)
     local layout_bg = csbNode:getChildByName("layout_bg")
     layout_bg:setTag(TAG_ENUM.BT_CLOSEHELP)
...
function GameViewLayer:createBankLayer()
    ...
    local editbox = ccui.EditBox:create(tmp:getContentSize(),"game_bank/im_bank_edit.png",0)
    ...
    editbox = ccui.EditBox:create(tmp:getContentSize(),"game_bank/im_bank_edit.png",0)
        :setPosition(tmp:getPosition())
        :setFontName("fonts/round_body.ttf")
        :setPlaceholderFontName("fonts/round_body.ttf")
        :setFontSize(24)
