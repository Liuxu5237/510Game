...
function GameViewLayer:initUserInfo(  )	
	...
	local head = g_var(PopupInfoHead):createClipHead(GlobalUserItem, tmp:getContentSize().width)
...
function GameViewLayer:initJettonBtnInfo(  )
	...
	self.m_bOtherPlayGoldFlyIn = true
	self.m_bMYGoldFlyIn = true
...
function GameViewLayer:onGetUserBet( )
	...
	local str = string.format("tabel_chip_%d_0.png", nIdx);
	local sp = nil
	...
	if nil ~= sp and nil ~= btn then
		...
		local act = self:getBetAnimation(self:getBetRandomPos(btn), cc.CallFunc:create(function()
			if wUser == self:getMeUserItem().wChairID then
				if self.m_bMYGoldFlyIn == true then
					self.m_bMYGoldFlyIn = false
					--播放下注声音			
					ExternalFun.playSoundEffect("ADD_SCORE.wav")
					RestoreSoundPlay(0)
				end
			else
				if self.m_bOtherPlayGoldFlyIn == true then
					self.m_bOtherPlayGoldFlyIn = false
					ExternalFun.playSoundEffect("ADD_SCORE.wav")
					RestoreSoundPlay(1)
				end
			end
...
function GameViewLayer:getBetRandomPos(nodeArea)
	...
	local nodeSize = cc.size(nodeArea:getContentSize().width / 2 - 100, nodeArea:getContentSize().height - 80);
...
function GameViewLayer:refreshJettonNode( node, my, total, bMyJetton )	
	if true == bMyJetton then
		...
		myscore = myscore - my
		local mystr = ExternalFun.numberThousands(myscore);
		self.m_textUserCoint:setString(mystr);
	end
