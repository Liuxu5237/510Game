...
function Cannon.getPos(bReversal,chairId)
	local pos = chairId  
    --print(bReversal and "反转喽" or "未反转")
...
--    end
    ...
    if self.m_nChairID<2 then--自己在上面0，1
	    if self.m_ChairID ~= self.m_nChairID and self.m_ChairID>=2 then
	       self.m_fort:setPosition(50,50)--这个炮台在下面
        else
           self.m_fort:setPosition(50,-5)--这个炮台在上面
        end
    else--自己在下面2，3
	    if self.m_ChairID ~= self.m_nChairID and self.m_ChairID<2 then
	       self.m_fort:setPosition(50,50)
...
--    	end
         showToast(cc.Director:getInstance():getRunningScene(), "上分不足,请充值！", 2)
        --
...
function Cannon:otherUpdate(dt)
    ...
    if fire.chair_id <2 or fire.chair_id ==7 then
       angle = angle+180
    ...
    if self.m_nChairID>1 then     --4,5,6同边位置  -->改成3,4,5,6同边
       if fire.chair_id>2 then            --4,5,6同边位置  -->改成3,4,5,6同边
          ifchange=true
       else
          ifchange=false
       end
    else
       if fire.chair_id<3 then             --1,2,3同边位置 -->改成1，2同边
          ifchange=true
       else
          ifchange=false
       end
