...
function QMyPlayer:ctor(vpos,delegate)
...
function QMyPlayer:playSendCardAction(cbCardData)
    local function removeThis()
        ...
        AudioEngine.playEffect("game/yule/oxsixex/res/sound/SEND_CARD.wav")
        self:showHandCardUI(cbCardData)
        self._delegate:showBtn(nil,true)
    end

    local posX,posY = 0,0
    for i,v in pairs(self.m_pImageBack_) do
        v:setVisible(true)
        posX = 585.0 + (i-1)*v:getContentSize().width/2
        posY = 10
...
function QMyPlayer:showHandCardUI(cbCardData)
    ...
    for i,v in pairs(cbCardData) do
        ...
        posX = 585.0 + (i-1)*pNode:getContentSize().width/2
        posY = 10
