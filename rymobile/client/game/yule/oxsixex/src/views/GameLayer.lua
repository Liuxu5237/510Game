...
function GameLayer:ctor(frameEngine,scene)
    ...
    if cc.FileUtils:getInstance():isFileExist(device.writablePath .. "game/yule/oxsixex/res/") then
        cc.FileUtils:getInstance():addSearchPath(device.writablePath .. "game/yule/oxsixex/res/",true)
    else
        cc.FileUtils:getInstance():addSearchPath("game/yule/oxsixex/res/",true)
    end
...
function GameLayer:onUserChangeTalbe() 
    ...
    for i = 1 ,cmd.GAME_PLAYER do
        viewID = self._gameView:GetPlayViewStation(i-1)
        self._gameView.m_pPlayers[viewID]:removeUI()
        self._gameView:showPlayerInfo(viewID,nil,false)
        self._gameView:showZhunBei(viewID,false)
    end
    self._gameView.m_pUserItem_ = {}
    self._gameView.m_nTableID = nil
    self._gameView.m_nChairID = nil  
...
function GameLayer:onBtnSendMessage(msgId,tData)
    ...
    elseif msgId == GameViewLayer.UiTag.eBtnStart then
        ...
        self._gameView.readynumber = self._gameView.readynumber + 1
        print("=================准备计数 = "..self._gameView.readynumber)
...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
    if GlobalUserItem.bIsChangeTableUser then
        ...
        self._gameView.m_pUserItem_ = self._gameFrame:GetMeUserItem()
        self._gameView.m_nTableID = self._gameView.m_pUserItem_.wTableID
        self._gameView.m_nChairID = self._gameView.m_pUserItem_.wChairID 
...
function GameLayer:onSubGameStart(dataBuffer)
    --self.m_tHandCardData_ = {} --游戏开始清理数据
