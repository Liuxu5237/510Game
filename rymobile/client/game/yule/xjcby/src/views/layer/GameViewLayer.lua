...
local module_pre = "game.yule.xjcby.src"	
...
function GameViewLayer:onExit()
    ...
    for k,v in pairs(oldPaths) do
        a = nil 
        a = string.match(tostring(v), "/xjcby/")
...
function GameViewLayer:addSerchPaths( )
   ...
   --搜索路径
    ...
    if device.platform == "windows" then
        cc.FileUtils:getInstance():addSearchPath("game/yule/xjcby/res/")
    end
...
function GameViewLayer:hideBtnSchedule()
    ...
    local function  update(dt)
        ...
        if self.m_touchTime and time - self.m_touchTime >= 30000 then
            if self.menu:isVisible() then
                self:unHideBtnSchedule()
                self.menu:setVisible(false)
                self.menuT:setVisible(true)

                self.exit:runAction(cc.MoveTo:create(0.2,cc.p(-50,630)))
                self.sound:runAction(cc.MoveTo:create(0.2,cc.p(-50,500)))
                self.help:runAction(cc.MoveTo:create(0.2,cc.p(-50,370)))
                self.btn_speed:runAction(cc.MoveTo:create(0.2, cc.p(-50,500)))
                self.byself:runAction(cc.MoveTo:create(0.2,cc.p(1380,515)))
                self.lock:runAction(cc.MoveTo:create(0.2,cc.p(1380,395)))
                self.upScore:runAction(cc.MoveTo:create(0.2,cc.p(1380,630)))
                self.downScore:runAction(cc.MoveTo:create(0.2,cc.p(1380,280)))
                self.ifmenu = false
            else
                self:unHideBtnSchedule()
            end
...
function GameViewLayer:initView(  )
   ...
   self.ifmenu = true
    ...
    self.byself:setPosition(cc.p(1150,515))
    ...
    self.lock:setPosition(cc.p(1150,395))
   ...
   :setVisible(false)
		--:setTag(UPSCORE)
    self.upScore:setPosition(cc.p(1150,630))
    self.upScore:setScale(0.9)
    ...
    :setVisible(false)
		--:setTag(DOWNSCORE)
	self.downScore:setPosition(cc.p(1150,280))
    self.downScore:setScale(0.9)
...
--菜单
    ...
    self.menu:addTouchEventListener(function (sender,eventType)
        if eventType == ccui.TouchEventType.ended  then
                self:unHideBtnSchedule()
                self.exit:runAction(cc.MoveTo:create(0.2,cc.p(-150,630)))
                self.sound:runAction(cc.MoveTo:create(0.2,cc.p(-150,500)))
                self.help:runAction(cc.MoveTo:create(0.2,cc.p(-150,370)))
                self.btn_speed:runAction(cc.MoveTo:create(0.2, cc.p(-150,500)))
                self.byself:runAction(cc.MoveTo:create(0.2,cc.p(1580,515)))
                self.lock:runAction(cc.MoveTo:create(0.2,cc.p(1580,395)))
                self.upScore:runAction(cc.MoveTo:create(0.2,cc.p(1580,630)))
                self.downScore:runAction(cc.MoveTo:create(0.2,cc.p(1580,280)))
                self.ifmenu = false
                self.menu:setVisible(false)
                self.menuT:setVisible(true) 
        end
    ...
    self.menuT:addTouchEventListener(function (sender,eventType)
        if eventType == ccui.TouchEventType.ended  then
            ...
            self:hideBtnSchedule()
            self.exit:runAction(cc.MoveTo:create(0.2,cc.p(150,630)))
            self.sound:runAction(cc.MoveTo:create(0.2,cc.p(230,500)))
            self.help:runAction(cc.MoveTo:create(0.2,cc.p(150,370)))
            self.btn_speed:runAction(cc.MoveTo:create(0.2, cc.p(90, 500)))
            self.byself:runAction(cc.MoveTo:create(0.2,cc.p(1150,515)))
            self.lock:runAction(cc.MoveTo:create(0.2,cc.p(1150,395)))
            self.upScore:runAction(cc.MoveTo:create(0.2,cc.p(1150,630)))
            self.downScore:runAction(cc.MoveTo:create(0.2,cc.p(1150,280)))
            self.ifmenu = true    
...
--退出
    ...
    self.exit:setPosition(cc.p(160,630))
    self.exit:setTag(4)
    self.exit:setScale(0.9)
...
--声音
    ...
    self.sound:setPosition(cc.p(230,500))
    self.sound:setScale(0.9)
    ...
    self.btn_speed:setPosition(cc.p(90, 500))
    self.btn_speed:setScale(0.9)
    ...
    self.btn_speed:addTouchEventListener(function(ref, tType)
        if tType == ccui.TouchEventType.ended then
            ...
            if ref:getTag() == 10 then
                ...
                self._scene.m_cannonLayer.m_bulletSpeed = 0.6		--加速速度      
    ...
    self.speed_effect:setScale(1.3)
...
--帮助
    ...
    self.help:setPosition(cc.p(150,370))
    self.help:setTag(2)
    self.help:setScale(0.9)
  ...
  nPos = CannonSprite.getPos(self._scene._dataModel.m_reversal,nPos)
   ...
   --金币类型
    ...
    if nil ~= anim then
        ...
        num:runAction(cc.Sequence:create(cc.DelayTime:create(1.0),cc.RemoveSelf:create(true)))
        ...
        for i=1,coinNum do
            ...
            coin:runAction(cc.Sequence:create(cc.MoveTo:create(tempTime, cannonPos), cc.RemoveSelf:create(true)))
        end
    ...
    if  fishtype ~= nil and fishtype >= Game_CMD.FishType.FishType_JianYu and fishtype <= Game_CMD.FishType.FishType_LiKui then
        ...
        if nil == reward then
            ...
            if nil ~= animation then
                ...
                reward:runAction(cc.Sequence:create(cc.DelayTime:create(time*16),cc.RemoveSelf:create(true)))
            end
