...
GameViewLayer.RES_PATH = "game/yule/twenty/res/"
...
function GameViewLayer:preloadUI()
	--添加路径
	--cc.FileUtils:getInstance():addSearchPath("game/yule/twenty/res/Jackpot/",true)
...
function GameViewLayer:registerTouch()
	...
	local function onTouchEnded( touch, event )
        ...
        if self.BigWinPanel:isVisible() then
            self:closeWinPanel()
        end
	end
...
--           												end), 10)
	...
	self.text_lines =self.pan_normal:getChildByName("text_lines") --总线
     ...
     -- self.m_textGoldPool:setScale(0.6)
      self.pan_normal:addChild(self.GameBalanceText)
 ...
 --    self.sp_logo:runAction(cc.RepeatForever:create(seq_gold))
    ...
    --初始化用户信息
    self.nickName = self.pan_normal:getChildByName("Text_1") --昵称
...
function GameViewLayer:goinFree()
	...
	self._self:runAction(cc.Sequence:create(cc.DelayTime:create(1.0),cc.CallFunc:create(function()
		local _BeginBtn = nil
...
function GameViewLayer:WildPosition(pos)
    ...
    local layout = cc.Sprite:create("Animation/11/item1.png")
    ...
    local LcurPosX = posX * 181 + 108
    local lCurPosY = posY * 168 + 47;
    if posX == 2 then
        LcurPosX = LcurPosX + 3
        lCurPosY = lCurPosY - 3
    end
    if posX == 3 then
        LcurPosX = LcurPosX + 12
        lCurPosY = lCurPosY - 4
    end
    if posX == 4 then
        LcurPosX = LcurPosX+25
        lCurPosY = lCurPosY-3
    end

    if posX == 5 then
        LcurPosX = LcurPosX+35
        lCurPosY = lCurPosY-3
    end
...
function GameViewLayer:playAnim()  --Scatter和wild动画
	...
	for i = 1 ,30 do 
		local str = string.format("Animation/12/item%d.png",i)
		animationWild:addSpriteFrameWithFile(str)
	end
	animationWild:setLoops(2) 
	animationWild:setDelayPerUnit(1.5/30)
	...
	for index,line in pairs(self.winLinePoints) do --显示点 
		for index2,point in pairs(line) do 
			if  self.tab[point[1]][point[2]] == "12" then ----可变动画
				...
				local rf =cc.RepeatForever:create(seq)
				local layout = cc.Sprite:create("Animation/12/item1.png")
