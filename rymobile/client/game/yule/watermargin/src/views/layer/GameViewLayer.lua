...
function Game1ViewLayer:NoticeCallBack( event )
...
function Game1ViewLayer:onEnterTransitionDidFinish()
...
function Game1ViewLayer:initUI( csbNode )
    ...
    --收分
    self.Button_GetScore = ccui.Button:create(GameViewLayer.RES_PATH .. "game2/game2_btn_get_01.png", GameViewLayer.RES_PATH .. "game2/game2_btn_get_02.png")
        :move(Button_Start:getPosition())
        :setTag(TAG_ENUM.TAG_GETSCORE_MENU)
        :setVisible(false)
        :addTo(csbNode)
    self.Button_GetScore:addTouchEventListener(btnEvent);  
	--显示菜单
	...
	--返回
	local Button_back = csbNode:getChildByName("Button_back");
	Button_back:setTag(TAG_ENUM.TAG_QUIT_MENU);
    Button_back:setPosition(56,702);
	Button_back:addTouchEventListener(btnEvent);
    --帮助
	local Button_Help = csbNode:getChildByName("Button_Help");
    ...
    if cc.PLATFORM_OS_WINDOWS == targetPlatform then
        local lBtnStart = csbNode:getChildByName("Button_Start");
        lBtnStart:runAction(cc.Sequence:create(cc.DelayTime:create(3),cc.CallFunc:create(function ()
    
            local listener = cc.EventListenerKeyboard:create()
            listener:registerScriptHandler(function(keyCode, event)
                if not self._scene.isGameTwo then
                    if keyCode == cc.KeyCode.KEY_SPACE then
                        if not self._isChickStart then
                            print("=============开始游戏==============")
                            self._isChickStart = true
                            self._scene:onGameStart()
                            self:game1ActionBanner(false)             --切换旗帜动作
                        end
                    end
                else
                    if keyCode == cc.KeyCode.KEY_DOWN_ARROW then--全比
                        self._scene:onAllIn()
                    elseif keyCode == cc.KeyCode.KEY_LEFT_ARROW then--半比
                        self._scene:onHalfIn()
                    elseif keyCode == cc.KeyCode.KEY_RIGHT_ARROW then--倍比
...
function Game1ViewLayer:setMyScore(SingleScore)
    local myscore = self.m_textScore:getString()
    myscore = tonumber(myscore)
    myscore = myscore - SingleScore
    self.m_textScore:setString(myscore)
end
--游戏1的通用动画
...
function Game1ViewLayer:BeginAnimatedMask()
    self.gamebegineffect = AudioEngine.playEffect("sound_res/gundong.wav")
    print("音乐ID = ", self.gamebegineffect)
    for i = 1, 15 do
...
function Game1ViewLayer:game1Begin(  )
    ...
    for i = 1, 15 do
        ...
        if node ~= nil then
			...
			local pItem =  node:getChildByTag(1)--GameItem:create()
			if pItem then
...
function Game1ViewLayer:game1End(  )
    ...
    self:stopAllActions()
    --音效停止
    if self.gamebegineffect ~= nil then
        AudioEngine.stopEffect(self.gamebegineffect)
        self.gamebegineffect = nil
    end
    self:game1Result()
	--切换旗帜动作
        						...
        						cc.CallFunc:create(function ()
        							...
        							if nodeTip then
                                        ...
                                        if self._scene.m_bEnterGame3 ~= true then
                                            self:updateStartOrGetScore(false)
                                        end
                                        --self:updateStartButtonState(true)
    	...
    	self:runAction(
    		cc.Sequence:create(
    			...
    			cc.CallFunc:create(function (  )
    				...
    				--游戏模式
    				self._scene:setGameMode(6) --GAME_STATE_END
    				--即将进入小玛丽
                    print("即将进入小玛丽")
    			...
    			self:runAction(
    				cc.Sequence:create(
                        ...
                        cc.CallFunc:create(function (  )
                            --if self._scene.m_bIsAuto == true then
                                self:enableGame2Btn(false)
                                self:updateStartOrGetScore(true)
                                local nodeTip = self._csbNode:getChildByName("Node_win")
...
function Game1ViewLayer:itemStopAction()
    --反复点击判断
    if self._isStopdisable then
        return
    end
    self._scene:setGameMode(6)
    
    self._isStopdisable = true
    for i = 1,15 do
        local posx = math.ceil(i/3)
        local posy = (i-1)%3 + 1
        local nodeStr = string.format("Node_%d_%d",posx-1,posy-1)
...
function Game1ViewLayer:onButtonClickedEvent(tag,ref)
	...
	elseif tag == TAG_ENUM.TAG_GAME2_BTN  then    --	开始游戏2
        self._csbNode:getChildByName("Node_win"):setVisible(false)
        self:updateStartOrGetScore(true)
        self:updateStartButtonState(true)
	...
	elseif tag == TAG_ENUM.TAG_SHOWUP_BTN  then   --显示上部菜单
		self:onShowTopMenu()
        ExternalFun.playClickEffect()
    elseif tag == TAG_ENUM.TAG_GETSCORE_MENU then --收分按钮
        --self:onGetScore()
        if self._scene.m_bIsAuto == false then
            self._scene:onGameStart()
        end
...
function Game1ViewLayer:setAutoStart( bisShow )
	...
	if spSelect then
        ...
        self._isChickStart = bisShow
        print(tostring(self._isChickStart))
	end
end
...
--开始按钮和收分按钮的切换
function Game1ViewLayer:updateStartOrGetScore(bIsShowStart)
    local Button_Start = self._csbNode:getChildByName("Button_Start")
    Button_Start:setVisible(bIsShowStart)
    self.Button_GetScore:setVisible(not bIsShowStart)
end

function Game1ViewLayer:onGetScore()  
    if self._isGetScoreDisable then
        return
    end
    print("收分")
    self._isGetScoreDisable = true
    local soundid = AudioEngine.playEffect("defen.wav")
    local nodeTip = self._csbNode:getChildByName("Node_win")
    local labNum = nodeTip:getChildByName("labNum")
    local _countDown = tonumber(labNum:getString())
    if not nodeTip:isVisible() then
        self._isChickStart = false
        return
    end
    local rate = math.ceil(_countDown / 20)  --根据不同的分数递减不同
    ...
    if nodeTip then
        self.entryId = cc.Director:getInstance():getScheduler():scheduleScriptFunc( function( dt )
                    ...
                    if _countDown <= rate then
                        ...
                        self:enableGame2Btn(false)
                        local _lCoins = self._scene.m_lCoins + self._scene.m_lGetCoins
                        self.m_textScore:setString(_lCoins)                      

                        AudioEngine.stopEffect(soundid)
                        self._isStopdisable = false
                        self._isGetScoreDisable = false
                        --self._isChickStart = false
                        return
                    end
                    _countDown = _countDown - rate
                    labNum:setString(_countDown)
                    end,0,false)
    end
end
...
function Game3ViewLayer:backOneGame()
	...
	self._scene:setGameMode(6) --GAME_STATE_WAITTING
    gameview:updateStartOrGetScore(false)
    --刷新分数
