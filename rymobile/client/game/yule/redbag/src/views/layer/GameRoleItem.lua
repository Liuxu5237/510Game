...
function GameRoleItem:ctor(userItem, viewId)
    ...
    head:enableInfoPop(false, popPosition[viewId], popAnchor[viewId])
    head:enableHeadFrame(false, {_framefile = "land_headframe.png", _zorder = -1, _scaleRate = 0.75, _posPer = cc.p(0.5, 0.63)})
    self.m_popHead = head
    ...
    self.m_spGameFrame:setTouchEnabled(false)

    -- 聊天气泡
    self.m_spChatBg = csbNode:getChildByName("chat_bg")
    self.m_spChatBg:loadTexture("public_res/pao.png")
...
function GameRoleItem:textChat( str )
    ...
    if nil == self.m_labChat then
        ...
        self.m_labChat:setColor(cc.c3b(0,0,0))      
    ...
    if labSize.height >= 40 then
        self.m_spChatBg:setContentSize(170, labSize.height + 40)
    else
        self.m_spChatBg:setContentSize(170, 45)
    end
    self.m_labChat:setPosition(self.m_spChatBg:getContentSize().width * 0.5, self.m_spChatBg:getContentSize().height * 0.5+4)
end
...
function GameRoleItem:browChat( idx )
    ...
    self.m_spBrow:setPosition(cc.p(40,48))
    self.m_spChatBg:setContentSize(80, 80)
...
function GameRoleItem:changeChatPos()
    if cmd.RIGHT_VIEWID == self.m_nViewId then
        ...
        self.m_spChatBg:loadTexture("public_res/pao2.png")
    ...
    elseif cmd.LEFT_VIEWID == self.m_nViewId then
        ...
        self.m_spChatBg:setAnchorPoint(cc.p(0, 0.5))
        self.m_spChatBg:loadTexture("public_res/pao.png")
        self.m_spChatBg:setPosition(13, 100)
    end
    self.m_spChatBg:stopAllActions()
