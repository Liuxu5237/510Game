	...
	function ZhuanPanAni:ctor(scene,begIndex,endIndex,duration,totalSec) --从最左边的第一个兔子开始为位置索引开始处，顺时针递增
		...
		self.tabZhuanpanPos = self.scene.aniPos
		self._time = 0.03
		self.count = 0
		self.index = 0
		self.cirleCount = 28
	...
	function ZhuanPanAni:ZhuanPan(callback)
		...
		--ExternalFun.playSoundEffect("zhuanzhuanzhuan.mp3")
		-- local delay = cc.DelayTime:create(self._time)
		-- local call = cc.CallFunc:create(function()
		-- 	self.count= self.count+1
		-- 	self.index = math.mod(self.index,self.cirleCount ) + 1

		-- 	if self.bright~=nil then
		-- 		self.bright:setPosition(self.tabZhuanpanPos[self.index] )				
		-- 	end
		...
		local function moveBrightRect(dt)
			...
			-- if self.nowCount <5 then
			-- 	self.frameN=self.frameN-0.3
			-- elseif self.nowCount >5 and self.nowCount < self.cirleCount*4 then
			-- 	self.frameN = self.frameN + 0.3
			-- if self.nowCount>self.cirleCount*4 then
			-- 	self.frameN = self.frameN-0.5
			-- end
			if self.frameN>=self.everyNFrame[self.speedKind] then
				...
				ExternalFun.playSoundEffect("zhuan5.mp3")
				--if self.nowCount < 4 then
					--ExternalFun.playSoundEffect("zhuan1.mp3")
				-- else
				-- 	ExternalFun.playSoundEffect("zhuan2.mp3")			
				--end
				--print(self.changeSpeedN)
				--print("spedd")
				--print(""..self.varSpeedSteps[self.speedKind])
				if self.changeSpeedN>=self.varSpeedSteps[self.speedKind] then
					...
					if self.changeSpeed>0  and #self.followRects<3 and self.changeSpeedN > 4 then
