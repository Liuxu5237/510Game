...
--控制
function GameLayer:SendContorl(lScore)
    local cmddata = ExternalFun.create_netdata(g_var(cmd).CMD_C_Contorl)
    cmddata:pushbyte(lScore)
    self:SendData(g_var(cmd).SUB_C_AMDIN_COMMAND, cmddata)
end


--更新库存
function GameLayer:DissfStoreScore(StoreScore)
    local cmddata = ExternalFun.create_netdata(g_var(cmd).CMD_S_UpdateStorage)
    cmddata:pushscore(StoreScore)
    self:SendData(g_var(cmd).SUB_C_CONTORLSCORE, cmddata)
end


--列表配置
function GameLayer:UserInfos()

    local cmddata = ExternalFun.create_netdata(g_var(cmd).CMD_S_OllUserInfo)
    cmddata.lStorageStart = 0
    self:SendData(g_var(cmd).SUB__C_CFISH_ROOM, cmddata)
end


--获取管理员权限
function GameLayer:SendAdminContorl()
    local cmddata = ExternalFun.create_netdata(g_var(cmd).CMD_C_Contorl)
    cmddata:pushbyte(cbRegionIndex)
    self:SendData(g_var(cmd).SUB_C_ADMIN, cmddata)
end
...
function GameLayer:onEventGameSceneFree( dataBuffer )
    ...
    --设置奖金池
    self._gameView:SetGlodPool(cmd_table.lGlodPool)
...
--    --申请条件
    self._gameView:onGetApplyBankerCondition(cmd_table.lApplyBankerCondition);
...
--    --玩家最大下注
    ...
    --self._gameView.ShowBankerBtn(false)
...
--    self._dataModle:calcuteRata(nWinCount, nLoseCount)
    ...
    for i = 1, cmd_table.cbCardCount[1][1] do
        tabRes.m_idleCards= cmd_table.cbTableCardArray[1][1]
    end
    for i=1,cmd_table.cbCardCount[1][2] do
        tabRes.m_masterCards= cmd_table.cbTableCardArray[1][2]
    end

    self._gameView:onGetGameCard(cmd_table.GetPoolScore, res.m_llTotal , tabRes, cmd_table.cbTimeLeave)
...
function GameLayer:onEventGameMessage(sub,dataBuffer)  
	...
	elseif sub == g_var(cmd).SUB_S_APPLY_BANKER then
        ...
        --库存更新
        elseif sub == g_var(cmd).SUB_S_CONTORLSCORE then
		self:onUpdateStorase(dataBuffer);
    elseif sub == g_var(cmd).SUB_S_ADMIN_USER then
		self:onSubAdminUser(dataBuffer);
    ...
    --在线列表
    elseif sub == g_var(cmd).SUB_S_CFISH_ROOM then
        self:onUserOffInfo(dataBuffer);
     --更新下注
    elseif sub == g_var(cmd).SUB_S_UserBetInfo then
        self:OnUpdateBet(dataBuffer);
...
function GameLayer:onSubPlaceJetton( dataBuffer )
    ...
    self._gameView:onGetUserBet(self.cmd_placebet.cbAndroidUser);
end
...
function GameLayer:onSubApplyBanker( dataBuffer )
    ...
    if self._gameView.m_applyListLayer then
        self._gameView.m_applyListLayer.b_isApplytouch = true
    end
end


--更新库存
function GameLayer:onUpdateStorase( dataBuffer )
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_UpdateStorage,dataBuffer);
    self._gameView:UpdateStore(cmd_table.lStorageStart)
end

--管理权限
function GameLayer:onSubAdminUser( dataBuffer )
 
 self._gameView:ShowAdminBtn()
end
...
function GameLayer:onSubSendRecord( dataBuffer )
    ...
    self._gameView:updateGameRecord(dataBuffer)
...
--在线列表
function GameLayer:onUserOffInfo( dataBuffer )

 local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_OFFUSER_INFO, dataBuffer) 
 self._gameView:onUserInfo(cmd_table)
end


--更新下注
function GameLayer:OnUpdateBet( dataBuffer )

 local cmd_table = ExternalFun.read_netdata(g_var(cmd).tagUserBetInfo, dataBuffer) 
 self._gameView:onBetInfo(cmd_table)
end
