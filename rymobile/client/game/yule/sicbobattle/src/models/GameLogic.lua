...
function GameLogic:DeduceWinner( nWinMultiple,cbDiceValue )
	...
	for cbIndex=1,GameLogic.DICE_COUNT do
		for cbRecord= cbIndex+1,GameLogic.DICE_COUNT-cbIndex do
			if analyseResult.m_DiceValue[cbRecord] == cbDiceValue[cbIndex] then
				...
				--print("cout是多少" .. analyseResult.m_DiceValue[cbIndex].cbCount)
				break
			end
	...
	--豹子判断 新加
	if analyseResult.m_DiceValue[1].cbDate == analyseResult.m_DiceValue[2].cbDate and 
		analyseResult.m_DiceValue[2].cbDate == analyseResult.m_DiceValue[3].cbDate then
		--闪亮豹子并移除单双大小闪亮
		nWinMultiple[8+analyseResult.m_DiceValue[1].cbDate]=150
		nWinMultiple[15]=24
