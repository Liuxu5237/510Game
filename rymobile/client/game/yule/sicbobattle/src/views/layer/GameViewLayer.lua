...
function GameViewLayer:initBtn( csbNode )
	...
	local function btnEvent( sender, eventType )
	 ...
	 local gold_icon =  self._csbNode:getChildByName("Sprite_icon_gold")
	 gold_icon:setTexture("plaza/game_gold.png")
	 gold_icon:setScale(0.5)
...
function GameViewLayer:initUserInfo( csbNode )
	...
	local head = g_var(PopupInfoHead):createClipHead(GlobalUserItem, tmp:getContentSize().width-20)
...
function GameViewLayer:reSetUserInfo( score )
	...
	--自己游戏币
	local lUserCoin = ExternalFun.numberThousands(self.m_scoreUser);
	
	print("自己游戏币:" .. lUserCoin)
	self.m_textUserCoin:setString(lUserCoin);
	--自己投注
	local lUserJetton = ExternalFun.numberThousands(self.m_lHaveJetton);
...
function GameViewLayer:refreshAppLyInfo()
	...
	for i=1,4 do
		...
		if userList[#userList-i+1] then
			...
			local scoreStr = ExternalFun.numberThousands(userList[#userList-i+1].m_userItem.lScore)
...
function GameViewLayer:initJettonBtnInfo( csbNode )
	...
	self.m_spriteSelect:setTexture("client/res/Jetton/bt_score_light.png")
	self.m_spriteSelect:setScale(0.6)
	self.m_spriteSelect:setPosition(cc.p(-100,43))
...
function GameViewLayer:refreshJetton(  )
	...
	local lAllJetton = ExternalFun.numberThousands(self.m_lAllJetton)
...
function GameViewLayer:onButtonClickedEvent(tag,ref)
	...
	elseif tag == TAG_ENUM.BT_RULE then  --帮助
    	...
    	self:getParentNode():getParentNode():popHelpLayer(url)
...
function GameViewLayer:showResult(  )
	...
	local myScore = ExternalFun.numberThousands(resultData.lUserScore)
	textMyScore:setString(myScore)

	--玩家返回积分
	local textMyRe = self.m_gameResultLayer:getChildByName("Text_myRe")
	local returnScore =ExternalFun.numberThousands(resultData.lUserReturnScore)
	textMyRe:setString(returnScore)
	--庄家分数
	local textBankerScore = self.m_gameResultLayer:getChildByName("Text_bankerScore")
	local bankerScore = ExternalFun.numberThousands(resultData.lBankerScore)
...
function GameViewLayer:onGetApplyBankerCondition( llCon , rob_config)
	...
	--self:refreshApplyScore(self.m_llCondition)
	--print("上装 条件 ： " .. self.m_llCondition)
	--超级抢庄配置
	-- if rob_config then
...
function GameViewLayer:onChangeBanker( _wBankerUser, _lBankerScore,_cbBankerTime ,_lBankerWinScore)
	...
	if lBankerWinScore then
		local chengJiStr = ExternalFun.numberThousands(lBankerWinScore);
...
function GameViewLayer:onGetUserBet( )
	...
	-- local a =0
	-- for i=1,#self.m_pJettonNumber do
	-- 	if nIdx ~= self.m_pJettonNumber[i].k then
	-- 		a = a+1
	-- 	end
	-- end
	-- if a>= #self.m_pJettonNumber then
	-- 	local b  = math.random(1,#self.m_pJettonNumber)
	-- 	nIdx = self.m_pJettonNumber[b].k
	-- end
	local sp = cc.Sprite:createWithSpriteFrameName(string.format("ChouMa_%d.png",self.m_pJettonNumber[nIdx].k))
	
	-- local str = string.format("client/res/Jetton/ChouMa_%d.png", self.m_pJettonNumber[nIdx].k);
