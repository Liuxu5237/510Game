...
function GameLayer:addEvent()
    ...
    local function eventListener(event)
        ...
        if self.ScoreCount ==0  then
            ...
            -- if roominfo ~= nil and roominfo.wServerType == 8 then 
            --     self.ScoreCount = 1000000
            -- end
...
function GameLayer:onSubExchangeScene(dataBuffer)
    ...
    if count >= 1 then
        ...
        -- 鱼阵数据
        exchangeScene = FishComb.scene_kind % 3
...
function GameLayer:onSubCreateMoreFishd(databuffer)
    print("===========> GameLayer:onSubCreateMoreFishd")
                ...
                if not self.m_cannonLayer then
                    ...
                    if fish ~= nil and fish.m_data ~= nil and fish.m_data.fish_id == catchData.dwFishID then
                        ...
                        if nil ~= fish then
                            ...
                            elseif fish.m_data ~= nil and fish.m_data.fish_kind >= Game_CMD.FishType_DaJinSha and fish.m_data.fish_kind <= Game_CMD.FishType_TuLongDao then
                                --大鱼震动
                                self:shockScreen()
                                --气泡
