...
function GameViewLayer:initCsbRes()
    ...
    _meusbt:addTouchEventListener(function(sender,eventType)
      if eventType == ccui.TouchEventType.ended then
        if self._bMenu == true then
          ...
          self._menus:runAction(cc.MoveTo:create(0.2, cc.p(98, 1065)))
        end
      end
    end)
    ...
    local score4 =self._rootNode:getChildByName(string.format("user_%d", 4)):getChildByName("txt_end_score")
     score4:setAnchorPoint(0,0.5)
     score4:setPositionX(-2)
     local score2 =self._rootNode:getChildByName(string.format("user_%d", 2)):getChildByName("txt_end_score")
     score2:setAnchorPoint(1,0.5)
     score2:setPositionX(138)
...
function GameViewLayer:initButtonEvent()
  ...
  local helpposx  = self._menus:getChildByName("bt_help"):getPositionX()
  local helpposy  = self._menus:getChildByName("bt_help"):getPositionY()
  local robotposx =  self._menus:getChildByName("bt_robot"):getPositionX()
  local robotposy =  self._menus:getChildByName("bt_robot"):getPositionY()
  local setposx =    self._menus:getChildByName("bt_set"):getPositionX()
  local setposy =    self._menus:getChildByName("bt_set"):getPositionY()
  --托管
  btn = self._menus:getChildByName("bt_robot")
  btn:setTag(TAG.MENU_TRUSTTEE)
  btn:setPosition(cc.p(helpposx,helpposy))
  btn:addTouchEventListener(handler(self, self.onEvent))
  ...
  btn:setPosition(cc.p(robotposx,robotposy))
  btn:addTouchEventListener(handler(self, self.onEvent))

  --退出
  btn = self._menus:getChildByName("bt_quit")
  btn:setPosition(cc.p(setposx,setposy))
...
function GameViewLayer:showMenu()
  ...
  local btpull = self._rootNode:getChildByName("bt_menu")
    if self._bMenu == true then
        self._menus:stopAllActions()
        self._menus:runAction(cc.MoveTo:create(0.2, cc.p(98, 402)))
    else
        self._menus:stopAllActions()
        self._menus:runAction(cc.MoveTo:create(0.2, cc.p(98, 1065)))
    end
...
function GameViewLayer:showChat()
  ...
  self._chatLayer:addTo(self, GameViewLayer.TopZorder)
...
function GameViewLayer:backRoom()
    ...
    if self._scene.m_cbGameStatus == cmd.GS_WK_PLAYING then
      self:getParentNode():onQueryExitGame("游戏中强退,将扣除10倍底注,是否确定退出?")
    else
...
function GameViewLayer:popSlectcardView()
  ...
  --选牌倒计时
  local time  =  self._scene.m_tTimeShowCard-5-- 服务器定义20 客户端有动作延迟了 写成15--(self._scene.m_tTimeShowCard == 0) and 60 or self._scene.m_tTimeShowCard
    ...
    for i=beginpos,beginpos+length do
      ...
      if nil ~= pcard then
        ...
        if i == midpos then
          ...
          -- else
            if cardType == GameLogic.CT_FIVE_STRAIGHT_FLUSH then
...
function GameViewLayer:showEndScore(endScore, bshow)
  ...
  for i=1,cmd.GAME_PLAYER do
    if self._scene._bUserGameStatus[i] == true then
      ...
      if userIndex == 4 then
        txtscore:runAction(cc.MoveTo:create(0.37, cc.p(-2, 212)))
      elseif userIndex == 2 then
        txtscore:runAction(cc.MoveTo:create(0.37, cc.p(138, 212)))
      else
