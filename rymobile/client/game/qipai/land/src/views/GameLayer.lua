...
function GameLayer:ctor( frameEngine,scene )  
    if cc.FileUtils:getInstance():isFileExist(device.writablePath .. "game/qipai/land/res/") then
        cc.FileUtils:getInstance():addSearchPath(device.writablePath .. "game/qipai/land/res/",true)
    else
        cc.FileUtils:getInstance():addSearchPath("game/qipai/land/res/",true)
    end      
...
function GameLayer:updatePromptList(cards, handCards, outViewId, curViewId)
    ...
    --移除出牌人手牌
    self._gameView:removeOutCard(curViewId)
...
function GameLayer:sendReady()
    ...
    --清空桌面 显示匹配tips
    self:reSetData()
    self._gameView:showWaitAni(true)

end

-- 发送领取红包
function GameLayer:sendGetHbAward( userid )

    local dataBuffer = CCmd_Data:create()
    dataBuffer:setcmdinfo(yl.MDM_GR_USER,yl.SUB_GR_RECEIVEREDPACKETS)
    dataBuffer:pushdword(userid)
    self._gameFrame:sendSocketData(dataBuffer)
    print("发红领取红包")
end

--拆红包
function GameLayer:sendCHbAward( )
    ...
    -- local dataBuffer = CCmd_Data:create()
    -- dataBuffer:setcmdinfo(3,8)
    -- self._gameFrame:sendSocketData(dataBuffer)
    -- print("发红领取红包")

    -- local cmddata = CCmd_Data:create()
    -- cmddata:pushword(self:GetMeChairID())
    -- self:SendData(cmd.SUB_C_HONGBAO,cmddata)
    -- print("拆红包")
...
--发送托管
function GameLayer:sendTrustee( bTrusteeship )
    self:KillGameClock()
    if bTrusteeship ==true then
        bTrusteeship = 1
    elseif bTrusteeship == false then
        bTrusteeship = 2
    end
    --bTrusteeship 1托管 2取消托管
    local cmddata = CCmd_Data:create(1)
    cmddata:pushbyte(bTrusteeship)
...
function GameLayer:onEventGameSceneFree( dataBuffer )
    ...
    -- local tmp_hb = cmd_table.szHongbao[1]
    -- yl.GAME_BOOSNUMBER = cmd_table.szTargetnumber
    -- yl.GAME_TARNUMBER = yl.GAME_BOOSNUMBER
    -- for i = 1, 3 do
    --     local chair = i - 1
    --     local viewId = self:SwitchViewChairID(chair)
    --     if cmd.MY_VIEWID == viewId then
    --         local hbcount = tmp_hb[i]
    --         if hbcount ~=nil then
    --             if yl.GAME_BOOSNUMBER <= tonumber(hbcount) then
    --                 yl.GAME_CURNUMBER  = yl.GAME_BOOSNUMBER
    --             else
    --                 yl.GAME_CURNUMBER =  tonumber(hbcount)
    --             end
    --         end
    --         self._gameView:updateHbCount()
    --     end
...
function GameLayer:onEventGameSceneCall( dataBuffer )
    ...
    for i = 1, 3 do
        ...
        if cmd.MY_VIEWID == viewId then
            ...
            --     end
            -- end
            --print("CurnGame HBCOUNT : " .. yl.GAME_CURNUMBER)
            self._gameView:updateHbCount()
        end
...
function GameLayer:onEventGameMessage(sub,dataBuffer)
    ...
    elseif cmd.SUB_S_GAME_CONCLUDE == sub then          --游戏结束
        ...
        self._gameView.temp_status = cmd.GAME_SCENE_END
...
function GameLayer:onSubTrustee(dataBuffer)
    ...
    --dump(cmd_table, "onTrustee", 6)

    --设置托管图标
    local curView = self:SwitchViewChairID(cmd_table.wTrusteeUser)
    if nil ~= curView and cmd.MY_VIEWID~=curView  then
        if cmd_table.bTrustee ==1 then
            self._gameView.m_userinfoControl:getChildByTag(curView):setVisible(true)
            self._gameView.m_tabUserHead[curView]:setVisible(false)
        elseif  cmd_table.bTrustee ==2 then
            self._gameView.m_userinfoControl:getChildByTag(curView):setVisible(false)
            self._gameView.m_tabUserHead[curView]:setVisible(true)
...
function GameLayer:onSubGameStart(dataBuffer)
    ...
    --发送局数
	-- local dataBuffer = CCmd_Data:create()
	-- dataBuffer:pushword(self:GetMeChairID())
    -- dataBuffer:pushbyte(tonumber(yl.GAME_CURNUMBER))
    -- dataBuffer:pushword(tonumber(yl.GAME_BOOSNUMBER))
    -- self:SendData(cmd.SUB_C_HONG_CONGT,dataBuffer)
...
function GameLayer:onSubOutCard(dataBuffer)
    ...
    --self._gameView:onChangePassBtnState(true)
     -- 不出按钮
     if #self.m_tabPromptList > 0 then
        self._gameView:onChangePassBtnState(not (curView == outView--[[#self.m_tabPromptList > 0]]))
    else
        self._gameView:onChangePassBtnState( true )
    end  
    ...
    local p_Cardtype = GameLogic:GetCardType( cmd_table.cbCardData[1],tonumber(cmd_table.cbCardCount))

    if p_Cardtype ~=nil then
        if p_Cardtype == GameLogic.CT_BOMB_CARD or p_Cardtype == GameLogic.CT_MISSILE_CARD then
            self._gameView:updataCellScore()
        end
        local tmp_handCards =self._gameView.m_tabNodeCards[outView]:getHandCards()
        --王炸移除所有出牌
        if p_Cardtype == GameLogic.CT_MISSILE_CARD and #tmp_handCards >0  then
            --print("王炸了")
            --print("id " .. outView)
            for i=1,3 do
                if i~= outView then
                    self._gameView:removeOutCard(i)
                end
            end
...
function GameLayer:onSubPassCard(dataBuffer)
    ...
    if self:IsValidViewID(curView) and self:IsValidViewID(passView) then
        ...
        if curView == cmd.MY_VIEWID then
            self._gameView:onGetOutCard(curView, curView, self.m_tabCurrentCards)
        else
            self._gameView:onGetOutCard(curView, curView, {})
        end
        -- 设置倒计时
...
function GameLayer:onSubGameConclude(dataBuffer)
    ...
    for i = 1, 3 do
        ...
        --结束时 手牌大于0  移除出牌
        if i ~=2 then
            --结束时 另外两个玩家的手牌数量   流程： 如果手牌不为0 清除出牌 然后亮牌 
            local tm_handcount = self._gameView:getHandCardCount(i)
            if tm_handcount~=0 then
                self._gameView:removeOutCard(i)
            end
            print("结算ID poke count " .. viewId .. count)
        end
...
function GameLayer:onSubSystemMessage( wType, szString)
    ...
    --self._gameView:updateTruMessage(wType,szString)
