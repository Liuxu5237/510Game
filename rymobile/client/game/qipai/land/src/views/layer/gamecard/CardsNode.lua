...
-- 非自己扑克尺寸
local CARD_HIDE_SCALE = 0.8
-- 亮牌尺寸
local CARD_LEFT_SCALE = 0.6
-- 非自己扑克牌 显示背面
local CARD_BACK_SCALE = 0.3
...
function CardsNode:onTouchEnded(touch, event)
	...
	self:dragCardsByZzy(vecSelectCard,1)
...
function CardsNode:updateCardsNode( cards, isShowCard, bAnimation, pCallBack)
	...
	local centerPos = cc.p(winSize.width * 0.5, winSize.height * 0.65)
	...
	if #cards <= self.m_cardsCount then
		for i = 1, self.m_cardsCount do
			...
			if nil ~= tmp then
				...
				--change 
				local pos = cc.p((i - center) * OPEN_CARD_X_DIS * CARD_HIDE_SCALE, 0)
				if i > 10 then
					pos = cc.p(((i - 10) - center) * OPEN_CARD_X_DIS * CARD_HIDE_SCALE, -50)
				end

				if cmd.RIGHT_VIEWID == self.m_nViewId then
					--完美兄弟
					--print("右边玩家开牌 " .. self.m_nViewId .. "手牌个数: " .. center)
					--center = center>10 and 10 or center-10
					local pos = cc.p( -(i * OPEN_CARD_X_DIS * CARD_HIDE_SCALE), 0)
					if i > 10 then
						pos = cc.p(- (i - center) * OPEN_CARD_X_DIS * CARD_HIDE_SCALE, -50)
					end
				...
				local moveTo = cc.MoveTo:create(0, pos)
			...
			for i = 1, count do		
				...
				if nil ~= tmp then
					...
					if tmp:getDispatched() then
						...
						local spa = cc.Spawn:create(moveTo, cc.ScaleTo:create(ANI_BEGIN,CARD_BACK_SCALE), seq)
...
--单独的选牌 弹出
function CardsNode:dragCardsByZzy( vecCard , isTouch )
	if type(vecCard) ~= "table"  then
		return
	end
	...
	if isTouch == 1 then
		...
		tagSearchCardResult = GameLogic:zzy_SelectCard(tmp_vecCard,#tmp_vecCard)
		if tagSearchCardResult~=nil then
			--dump(tagSearchCardResult)
			if tagSearchCardResult[3][1] ~= nil then
				self.m_mapSelectedCards = {}
				tmp_vecCard = tagSearchCardResult[3][1]

				for k,v in pairs(vecCard) do
					for k1,v1 in pairs(tmp_vecCard) do
						if v:getCardData() == v1 then
							tmp_vecCard2[k] = v
						end
					end
				end
			
				vecCard = tmp_vecCard2
