...
function GameLogic:CompareCard(cbFirstCard,cbFirstCount,cbNextCard,cbNextCount)
    ...
    --开始对比
    if (cbNextType == GameLogic.CT_SINGLE) or (cbNextType == GameLogic.CT_DOUBLE) or (cbNextType == GameLogic.CT_THREE) or (cbNextType == GameLogic.CT_SINGLE_LINE) or (cbNextType == GameLogic.CT_DOUBLE_LINE) or (cbNextType == GameLogic.CT_THREE_LINE)  then
    ...
    elseif cbNextType == GameLogic.CT_BOMB_CARDthen then
        local cbNextLogicValue = GameLogic:GetCardLogicValue(cbNextCard[1])
        local cbFirstLogicValue = GameLogic:GetCardLogicValue(cbFirstCard[1])
        return cbNextLogicValue > cbFirstLogicValue
...
--滑动智能选牌 test1 
function GameLogic:zzy_SelectCard(cbHandCardData,cbHandCardCount) 
    ...
    local cbCardCount = cbHandCardCount
    --出牌分析
    local cbTurnOutType = GameLogic:GetCardType(cbHandCardData, cbHandCardCount)
    if cbTurnOutType == GameLogic.CT_ERROR then --错误类型
