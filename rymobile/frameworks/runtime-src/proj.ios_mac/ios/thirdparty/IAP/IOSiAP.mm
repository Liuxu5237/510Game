     ...
     didReceiveResponse:(SKProductsResponse *)response
{
    ...
    for (int index = 0; index < [response.products count]; index++) {
        ...
        iosProduct->localizedTitle = "buy zuangshi"/*std::string([skProduct.localizedTitle UTF8String])*/;
        iosProduct->localizedDescription = "more zuanshi"/*std::string([skProduct.localizedDescription UTF8String])*/;
...
IOSProduct *IOSiAP::iOSProductByIdentifier(std::string &identifier)
{
    ...
    {
        IOSProduct *iosProduct = *iterator;
        NSLog(@"productIdentifier:%s identifier:%s",iosProduct->productIdentifier.c_str(),identifier.c_str());
