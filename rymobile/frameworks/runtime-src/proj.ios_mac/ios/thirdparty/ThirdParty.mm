...
- (void)thirdAppOpenFailure
{
    if (nil != _maiDelegate)
    {
        [_maiDelegate onMaiNotify:@"" backMsg:@"第三方应用打开失败"];
    }
}
...
- (void)openWebFailer:(NSString *)failer
{
    if (nil != _maiDelegate)
    {
        [_maiDelegate onMaiNotify:@"" backMsg:failer];
    }
}
#pragma mark -
...
- (void) thirdPartyMai:(NSString *)plat delegate:(id<MaiDelegate>)delegate payparam:(NSDictionary *)payparam
{
    _payPlat = plat;
    [self setMaiDelegate:delegate];
    
    if ([WECHAT isEqualToString:plat])
    {
        [self doWeChatMai:[payparam objectForKey:@"info"]];
    }
...
- (void) getPayList:(NSString *)token delegate:(id<MaiDelegate>)delegate
{
...
#pragma mark -
- (void) doWeChatMai:(NSString*) infostr
{
    ...
    req.sign = sign;
    //NSLog(@"partnerId=%@, prepayId=%@, nonceStr=%@, package=%@, timeStamp=%d, sign=%@",req.partnerId, req.prepayId, req.nonceStr,req.package,req.timeStamp,req.sign );
    [WXApi sendReq:req];
}
/*
...
- (void) onPayResult:(BOOL)res msg:(NSString*)msg
{
    if (nil != _maiDelegate)
    {
        if (TRUE == res)
        {
            [_maiDelegate onMaiSuccess:_payPlat backMsg:msg];
        }
        else
        {
            [_maiDelegate onMaiFail:_payPlat backMsg:msg];
        }
    }
    [self setMaiDelegate:nil];
}
@end
