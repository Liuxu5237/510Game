...
namespace WidgetFun
{
	...
	cocos2d::Sprite* createSprite(const char *kFileName)
	{
		...
		if (!pSprite)
		{
			pSprite = Sprite::create("PlatForm/Global/Error.png");
			CCAssert(pSprite,"");
		}
		return pSprite;
	}
	...
	void setTextStr(cocos2d::Node* pNode,std::string kTxt)
	{
		...
		pLabel->setString(a_u8(kTxt.c_str()));
	}
	...
	void setImagic(cocos2d::Node* pNode, std::string kImagic, int bSame, bool Isplist)
	{
		CCAssert(pNode,"");
		...
		if (Isplist == true)
		{
			CCSpriteFrame* frame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(kImagic);
			pImagic->setDisplayFrame(frame);
		}
		else
		{
	...
	void setImagic(cocos2d::Node* pNode, std::string kName, std::string kImagic, bool bSame, bool Isplist)
	{
		setImagic(getChildWidget(pNode, kName), kImagic, bSame, Isplist);
	}
	...
	void setAtlasTxt(cocos2d::Node* pNode,std::string kTxt)
	{
		...
		pAtlas->setString(a_u8(kTxt.c_str()));
	}
