...
class BezierToEx : public cocos2d::BezierTo
{
public:
	static BezierToEx* create(float t, const ccBezierConfig& c);
public:
	void setDest(NodeEx* pNode,cocos2d::Point kOffectDestPos);
	float bezierat( float a, float b, float c, float d, float t );
	void setCallFun(std::function<void(const Vec2&)>  pFun);
	virtual void update(float time) override;
private:
	FvReferencePtr<NodeEx> m_pDest;
	cocos2d::Point			m_kOffectDestPos;
	std::function<void(const Vec2&)> m_pCallFun;
};

class MoveToEx : public ActionInterval
{
public:
	static MoveToEx* create(float duration,cocos2d::Point kPos);
public:
	void setDest(NodeEx* pNode,cocos2d::Point kOffectDestPos);
	void setDestPos(cocos2d::Point kPos);
	void setCallFun(std::function<void(const Vec2&)>  pFun);
	virtual void startWithTarget(Node *target) override;
	virtual void update(float time) override;
private:
	FvReferencePtr<NodeEx>	m_pDest;
	cocos2d::Point			m_kOffectDestPos;
	cocos2d::Point			m_kDestPos;
	cocos2d::Point			m_kStartPos;
	std::function<void(const Vec2&)> m_pCallFun;
};
