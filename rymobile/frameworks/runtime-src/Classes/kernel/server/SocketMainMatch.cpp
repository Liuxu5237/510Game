...
/*  比赛消息处理                                                        */
/************************************************************************/
bool CServerItem::OnSocketMainMatch(int sub, char * data, int dataSize)
{
	...
	if (pEngine){
		...
		return luaStack->call_script_fun("gameMatchInterface.OnSocketMainMatch", sub, (cocos2d::Ref *)&packet, dataSize) > 0 ? true : false;
	}
