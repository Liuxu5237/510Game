...
//数组:utf8 -> unicode
WORD YYConverUtf8ToUnicodeWithArray(TCHAR destUnicode[], int tcharLength, c_char srcUtf8[])
{
    if(srcUtf8==NULL) return 0;
...
void CDownAsset::DownRun()
{
	...
	do
	{
        ...
        const int nLen = (int)outFileName.length();
        TCHAR* pTempText = new TCHAR[nLen];
        WORD wTempLen = YYConverUtf8ToUnicodeWithArray(pTempText, nLen, outFileName.c_str());
        
    #if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
        fp= _wfopen_s(wTempLen, L"wb+");
    #else
