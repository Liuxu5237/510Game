...
function RoomCardShopLayer:initBuyUI(csbNode)
    ...
    if nil ~= self.m_tabParam.item.bean and 0 ~= self.m_tabParam.item.bean then
        cc.Label:createWithTTF(string.formatNumberThousands(self.m_tabParam.item.bean,true,",").."游戏钻", "fonts/round_body.ttf", 24)
...
function RoomCardShopLayer:onUpdatePrice()
    if CBT_BEAN == self.m_nSelect then
        self.m_txtItemPriceType:setString("游戏钻/个")
        self.m_txtBuyPriceType:setString("游戏钻")
        self.m_txtDiscountPriceType:setString("游戏钻/个")
